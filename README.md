...
- 초대 링크는 `/auth/invite`에서 **초대 수락**을 눌러야 인증됩니다. 미리보기나 페이지 열기만으로는 일회용 토큰을 소비하지 않습니다. 수락했지만 비밀번호가 없는 초대 viewer는 같은 이메일로 **설정 이어가기** 링크를 재발급할 수 있습니다.
...
- viewer는 출생년도·키·거주지·직장을 묶은 프로필 제목으로 표시되며, 본인과 반대 성별의 활성 인물만 기본 표시
- 관리자용 매칭 생성, 결과 변경, 삭제
- 매칭 인물 선택 목록은 인물 목록과 같은 순서로 표시되고, 이름이 없는 인물의 남자/여자 번호도 동일하게 유지
- 목록·매칭 카드·매칭 선택창·매칭 히스토리의 긴 이름은 한 줄 말줄임으로 표시합니다. 출생년도·거주지와 viewer 프로필 제목·미리보기는 단어 경계를 유지하며 줄 길이를 고르게 배분합니다(`text-wrap: balance` 지원 브라우저). 인물 상세 헤더에서는 이름 전문을 확인할 수 있습니다.
- 인물 상세 페이지의 매칭 히스토리
...
- `viewer`: 이름/사진이 제외된 `people_public` 뷰만 볼 수 있고, 화면에서는 `1995년생 · 178cm · 서울 · 공기업`처럼 출생년도·키·거주지·직장을 제목으로 표시합니다. 없는 항목은 생략하고 모두 없으면 `프로필`로 표시합니다. 나이와 인물 번호는 표시하지 않으며 성별은 왼쪽 아바타에 `남`/`여`로 한 번만 표시합니다. 목록의 제목과 `이상형 · 내용` 미리보기는 각각 최대 두 줄이며 넘치면 말줄임 처리하고, 한글은 글자 중간이 아닌 단어 경계에서 줄바꿈합니다. 이상형이 없으면 미리보기도 숨깁니다. 상세에서는 제목과 이상형 전문을 표시하고 이상형의 줄바꿈을 보존합니다. 관리자 화면의 이름·사진 표시는 유지되며 viewer는 매칭 기록을 볼 수 없습니다.
...
├── pages/
│   ├── AuthCallback.tsx
│   ├── AcceptInvitePage.tsx # 명시적 초대 수락 후 세션 생성·비밀번호 설정
│   ├── Layout.tsx
│   ├── LoginPage.tsx
│   ├── MatchesPage.tsx
│   ├── PeoplePage.tsx
...
npm run test:invite:db      # 격리된 메모리 PostgreSQL에서 재발급 SQL 조건·실행 권한 검사
...
4. 상대가 링크에서 **초대 수락**을 누르면 앱이 `verifyOtp`로 세션 생성을 완료한 뒤 비밀번호·성별 설정 화면으로 이동합니다. 설정을 마치면 `viewer`로 이용할 수 있습니다.
...
설정 완료 여부에 따라 로그인 또는 새 링크 요청을 안내합니다. **로그인으로 이동**을 누르면
로그인된 브라우저에서는 사람 목록으로, 로그인되지 않은 브라우저에서는 로그인 화면으로 이동합니다.
설정을 마친 뒤에는 초대 링크를 다시 사용하지 않고 앱 주소로 접속합니다.
이미 수락했지만 비밀번호가 한 번도 설정되지 않은 초대 viewer는
같은 이메일로 링크를 만들면 **설정 이어가기** 링크를 받습니다. Supabase는 초대 수락 때 임시
비밀번호를 자동 생성하므로 해시 존재만으로 설정 완료를 판단하지 않습니다. 서버 전용 테이블과
Auth 트리거가 그 이후의 실제 비밀번호 변경을 기록합니다. 서버 함수는 초대·이메일 확인 이력,
미완료 상태, viewer 역할, 차단 여부를 확인하고 Auth의 recovery 토큰을 발급합니다.
생성 후에도 자격을 다시 검사합니다. `password_set` 같은 사용자가 바꿀 수 있는
메타데이터는 재발급 권한 근거로 사용하지 않습니다. 이미 본인 비밀번호를 설정한 계정, 관리자,
초대 이력이 없는 계정에는 링크가 발급되지 않습니다. 이전 버전에서 이미 수락한 계정은 당시의
비밀번호 변경 기록을 알 수 없으므로 기본적으로 재발급을 막고, 중단된 초대임을 개별 확인한 뒤
운영자가 서버의 미완료 상태를 복구해야 합니다. 기존 계정을 일괄 미완료로 처리하지 않습니다.

공유 링크의 토큰은 `#` 뒤에 있어 페이지 GET 요청이나 서버 접근 로그로 전달되지 않습니다.
카카오톡 등의 미리보기가 앱 페이지를 읽어도 인증 API를 호출하지 않고, 수락 버튼에서만 토큰을
교환합니다. 인증 성공 후 주소의 토큰을 지웁니다. 카카오톡 내장 브라우저와 외부 브라우저/PWA는
세션 저장소가 다를 수 있으므로 수락한 브라우저에서 설정을 마칩니다.
...
기존 메일/콜백 링크와의 호환을 위해 이 주소를 유지합니다. 새 공유 링크는 서버의 `APP_URL`을
사용한 `/auth/invite#token_hash=...&type=...` 형식이며 앱에서 토큰을 직접 교환하므로
`/auth/invite`나 `/set-password`를 Redirect URLs에 추가할 필요가 없습니다.
함수는 요청에서 받은 redirect나 역할을 사용하지 않습니다.
...
신규 사용자를 `viewer`로 만드는 기존 `20260726000000_harden_role_access.sql`의 트리거와
RLS 적용이 전제입니다. `20260910000000_allow_incomplete_invite_setup.sql`은
서버 전용 `invite_setup_state` 테이블, `auth.users`의 비밀번호 변경 추적 트리거,
`public.incomplete_invited_viewer_id(text)` 함수를 추가합니다. 상태 테이블은 RLS와 권한 회수로
브라우저 접근을 차단하며 판정 함수는 `service_role`에만 실행을 허용합니다.
사용자·기존 비밀번호·기존 역할을 변경하지 않습니다. 과거 비밀번호 해시가 있는 초대는 보수적으로
완료 상태로 분류하고, 재실행 시에도 이미 기록된 상태를 초기화하지 않습니다.
...
기존 버전에서 업데이트할 때는 **프런트엔드 배포 → 새 migration 적용 → 함수 재배포** 순서로
진행합니다. 프런트엔드에 `/auth/invite`가 없는 상태에서 함수를 먼저 배포하면 새 링크를
처리할 화면이 없어 로그인으로 표시될 수 있습니다. 프런트엔드 배포 후 PWA/관련 탭을 모두
닫고 다시 열어 최신 앱을 사용합니다. 새 migration은 Supabase SQL Editor에서 해당 SQL 파일
내용을 실행하거나, CLI migration 이력을 관리하는 환경이면 `npx supabase db push`로 적용합니다.
기존 migration을 무조건 다시 적용하지 않습니다. 함수는 앞의 `npx supabase functions deploy`
명령으로 재배포하고, 초대 링크도 앱에서 새로 생성합니다.

- 테스트용 관리자 계정으로 링크를 생성하고 새 브라우저에서 열기 → 수락 버튼 → 비밀번호·성별 설정 → viewer 접근을 확인합니다. 페이지를 열기만 했을 때 인증 요청이 없어야 하고 이름·사진·매칭 관리 권한이 생기지 않아야 합니다.
- 미수락 테스트 계정은 같은 이메일로 재생성할 수 있고, 수락했지만 비밀번호가 없는 초대 viewer는 설정 이어가기가 가능해야 합니다. 비밀번호가 있는 계정은 `409 already_registered`로 거절되는지 확인합니다. 사용된 링크와 만료된 링크는 새 링크 요청 안내를 보여야 합니다.
...
SQL 권한·비밀번호/역할별 판정은 migration 적용 후 로컬/테스트 DB에서
`supabase/tests/invite_setup.sql`로 확인할 수 있으며 테스트 사용자 변경은 모두 롤백합니다.
DDL 적용 후 Supabase security/performance advisors도 확인합니다.
